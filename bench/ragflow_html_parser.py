...
# --- vendored shim: the parser's only coupling to the RAGFlow repo is these
# --- two helpers. Replaced 1:1 so the parser's own logic runs unchanged.
import chardet as _cd


def find_codec(b):
    return (_cd.detect(b) or {}).get("encoding") or "utf-8"


class _Tok:
    # RAGFlow's rag_tokenizer.tokenize returns a space-joined STRING, not a
    # list; _token_count() calls .split(" ") on the result.
    @staticmethod
    def tokenize(t):
